...
RUN ["/bin/bash", "-c", "cat $FLINK_DIRECTORY/bin/jobmanager.sh | replace-string $FLINK_TASKMANAGER_SCRIPT_TO_REPLACE $FLINK_TASKMANAGER_SCRIPT_REPLACE_WITH > $FLINK_DIRECTORY/bin/taskmanager-blocking.sh && \
