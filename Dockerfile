...
RUN ["/bin/bash", "-c", "cat $FLINK_DIRECTORY/bin/jobmanager.sh | python -c \"import sys;print ''.join(map(lambda line: line.replace('$FLINK_TASKMANAGER_SCRIPT_TO_REPLACE', '$FLINK_TASKMANAGER_SCRIPT_REPLACE_WITH'), sys.stdin.readlines())); \" > $FLINK_DIRECTORY/bin/jobmanager-blocking.sh && \
